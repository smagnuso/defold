...
static WGPURenderPipeline WebGPUGetOrCreateRenderPipeline(WebGPUContext* context)
{
    ...
    if (context->m_CurrentRenderPass.m_Target->m_TextureDepthStencil)
    {
        ...
        depthstencil_desc.depthCompare = g_webgpu_compare_funcs[context->m_CurrentPipelineState.m_DepthTestFunc];
...
void WebGPUContext::requestDeviceCallback(WGPURequestDeviceStatus status, WGPUDevice device, const char *message, void *userdata)
{
    ...
    if (device)
    {
        ...
        WebGPUCreateSwapchain(context, context->m_Width, context->m_Height);
    }
    else
    {
...
static void WebGPUBeginFrame(HContext _context)
{
    ...
    WebGPUContext* context = (WebGPUContext*) _context;
    {
        const uint32_t widthWindow = GetWindowWidth(context->m_Window), heightWindow = GetWindowHeight(context->m_Window);
        if (!context->m_MainRenderTarget || widthWindow != context->m_MainRenderTarget->m_Width || heightWindow != context->m_MainRenderTarget->m_Height) // (re)create
            WebGPUCreateSwapchain(context, widthWindow, heightWindow);
...
static void WebGPUFlip(HContext _context)
{
    ...
    if (context->m_CommandEncoder)
    {
        ...
        wgpuCommandEncoderReference(context->m_CommandEncoder);
